...
import { Shield, Key, BadgeCheck, Lock, WifiOff, Eye, EyeOff } from 'lucide-react';

interface LoginProps {
  onLogin: (officerId: string, role: string) => void;
}
    ...
    <div className="min-h-screen bg-surface-dim flex flex-col items-center justify-center p-6 relative select-none">
      ...
      <div className="w-full max-w-md mb-6 z-10 flex items-center justify-between px-4 py-2 bg-surface rounded-xl border border-outline shadow-sm">
        ...
        <div className="flex items-center gap-1.5">
          <button
            type="button"
            onClick={() => applyPersona('SHARMA-994', 'Auditor - Supervisory Lead', 'Alpha-Secure-901')}
            className={`px-2.5 py-1 text-[11px] font-medium rounded-lg transition ${
              officerId === 'SHARMA-994'
          ...
          <button
            type="button"
            onClick={() => applyPersona('ROOT-SEC-01', 'System Administrator', 'Node-Admin-004')}
            className={`px-2.5 py-1 text-[11px] font-medium rounded-lg transition ${
              officerId === 'ROOT-SEC-01'
                ? 'bg-primary-container text-primary font-bold'
                : 'bg-surface-container-high text-on-surface hover:bg-surface-container-highest'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => applyPersona('DIR-GEN-07', 'NCIIPC Director / General Counsel', 'NCIIPC-Exec-77')}
            className={`px-2.5 py-1 text-[11px] font-medium rounded-lg transition ${
              officerId === 'DIR-GEN-07'
                ? 'bg-primary-container text-primary font-bold'
                : 'bg-surface-container-high text-on-surface hover:bg-surface-container-highest'
            }`}
          >
      ...
      <div className="w-full max-w-md bg-surface rounded-2xl p-8 border border-outline shadow-md relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <h1 className="text-[18px] font-bold text-on-surface tracking-tight uppercase">
            NCIIPC SECURE TERMINAL
          </h1>
          ...
          <div className="mt-3 px-3 py-1 bg-surface-dim rounded-full flex items-center gap-2 border border-outline">
            ...
            <span className="text-[10px] font-mono text-emerald-700 font-bold uppercase tracking-wider">
              ENCLAVE LEVEL-5 // OFFLINE NODE: 127.0.0.1
            </span>
          </div>
        </div>
        ...
        <p className="text-center text-[11px] text-on-surface-variant mt-5">
          TPM 2.0 Validated · Zero External Telemetry Leakage
        </p>
      </div>
    </div>
  );
